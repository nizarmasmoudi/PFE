...
class Image:
    def __init__(self, img_path, format='matplotlib'):
        if os.path.exists(img_path):
            self.img_path = img_path
        else:
            raise FileNotFoundError('Image {} Not Found!'.format(img_path))
        self.id = img_path.split('\\')[-1].split('.')[0]
        self.img = cv2.imread(self.img_path) if format == 'cv2' else plt.imread(self.img_path)
    ...
    def display(self, draw_bbox=False):
        if draw_bbox:
            annotations = self.read_annotations(subclasses=[1, 2, 0], subset=['left', 'top', 'width', 'height', 'object_category'])
            for left, top, width, height, obj in annotations.values:
                if obj in [1, 2]:
                    ...
                    img_ = self.img.copy()
                    img_[top:top+height, left:left+width] = patch        
            cv2.imshow(self.id, img_)
            cv2.waitKey(0)
        else:
    ...
    def split(self, overlap=0, save_output={'images': None, 'annotations': None}):
        ...
        annotations = self.read_annotations(subset=['left', 'top', 'width', 'height'])
        bottom_right, top_right, bottom_left, top_left= [], [], [], []
        for annotation in annotations.values:
            bbox_left, bbox_top, bbox_width, bbox_height = annotation
